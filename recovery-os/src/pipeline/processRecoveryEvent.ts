...
export async function processRecoveryEvent(eventId: string): Promise<void> {
  ...
  try {
    ...
    const existingDiagnosis = await client.query(
    ...
    const existingIntervention = await client.query(
    ...
    await client.query("SELECT pg_advisory_unlock(hashtext($1))", [eventId]);
    client.release();
  }
}
