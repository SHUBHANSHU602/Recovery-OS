...
async function main() {
  ...
  const listed = await listRecoveryCases(pool, { search: "dashboard-test@example.com" });
  assert.equal(listed.cases.length, 1, "case search should find the dashboard test customer");
