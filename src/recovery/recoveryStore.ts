...
async function initializeTrack3Schema(pool: Pool): Promise<void> {
  const client = await pool.connect();
  try {
    await client.query("BEGIN");
    await client.query("SELECT pg_advisory_xact_lock($1)", [TRACK3_SCHEMA_LOCK_ID]);
    await client.query(`
...
export async function ensureTrack3Schema(pool: Pool): Promise<void> {
  if (!schemaInitialization) {
    schemaInitialization = initializeTrack3Schema(pool).catch((error) => {
